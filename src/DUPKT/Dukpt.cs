using DeviceDecryptorTool.DukptNet.Test;
using System;
...
namespace DeviceDecryptorTool.DukptNet
{
    public static class Dukpt
    {
        ...
        private static BigInteger CreateSessionKey(string bdk, string ksn, DUKPTVariant DUKPTVariant)
        {
            ...
            Debug.WriteLine(string.Format("PEK_: {0}", Helpers.ByteArrayToHexString(BigIntegerExtensions.GetBytes(ipek))));
            ...
            Debug.WriteLine(string.Format("KEY_: {0}", Helpers.ByteArrayToHexString(BigIntegerExtensions.GetBytes(sessionKey))));
        ...
        public static byte[] Decrypt(string bdk, string ksn, byte[] encryptedData, DUKPTVariant variant)
        {
            ...
            Debug.WriteLine(string.Format("BDK_: {0}", bdk));

            if (string.IsNullOrEmpty(ksn))
            {
                throw new ArgumentNullException(nameof(ksn));
            }
            Debug.WriteLine(string.Format("KSN_: {0}", ksn));

            if (encryptedData == null)
            {
                throw new ArgumentNullException(nameof(encryptedData));
            }
            Debug.WriteLine(string.Format("DATA: {0}", Helpers.ByteArrayToHexString(encryptedData)));
