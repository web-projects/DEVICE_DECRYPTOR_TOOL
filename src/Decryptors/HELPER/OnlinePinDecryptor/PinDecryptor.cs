...
namespace Decryptors.HELPER.OnlinePinDecryptor
{
    ...
    public class PinDecryptor : IPinDecryptor
    {
        ...
        public byte[] DecryptData(string initialKSN, string cipher)
        {
            ...
            //sessionKey = CreateDataSessionKey(iPEK, baseKSN);
            sessionKey = CreatePinSessionKey(iPEK, baseKSN);
        ...
        /// <summary>
        /// PIN information retrieval after decrypted output production
        /// Use resulting PIN Block and PAN with 'F' as padding character to decode
        /// Example
        /// PIN Block: 044326CFF768AFFE
        /// PAN      : 5413330089750013
        /// DECODED  : 4315
        /// </summary>
        /// <param name="pinInformation"></param>
        /// <returns></returns>
        public OnlinePinData RetrievePinData(string panData, byte[] pinInformation)
        {
            ...
            int pinLength = pinInformation[0] & 0x0f;
            ...
            // check for odd pin length and adjust accordingly
            if ((pinLength % 2) == 1)
            {
                pinLength ++;
            }

            // Adjust remaining pin length to decode
            pinLength /= 2;

            // Skip the first 3 bytes, take last 12 bytes from pan, and reverse the resulting bytes
            byte[] panDataArray = ConversionHelper.HexToByteArray(panData.Substring(3, 12));
            Array.Reverse(panDataArray, 0, 6);

            // reverse encoded pin array
            byte[] encodedPin = new byte[pinInformation.Length];
            Array.Copy(pinInformation, encodedPin, pinInformation.Length);
            Array.Reverse(encodedPin, 0, encodedPin.Length);

            int appendCount = encodedPin.Length - panDataArray.Length;
            byte[] pinDecodedArray = new byte[panDataArray.Length + appendCount];

            int i = 0;

            for (; i < panDataArray.Length; i++)
            {
                byte val = encodedPin[i];
                val ^= panDataArray[i];
                pinDecodedArray[i] = val;
            }
            ...
            {
                appendCount --;
                pinDecodedArray[i++] = encodedPin[appendCount];
            }

            Array.Reverse(pinDecodedArray, 0, pinDecodedArray.Length);

            byte[] clearPin = new byte[pinLength];
            Array.Copy(pinDecodedArray, 1, clearPin, 0, pinLength);

            // collect pin values from data in the clear
            pinData.PANData = ConversionHelper.ByteArrayToHexString(clearPin);
