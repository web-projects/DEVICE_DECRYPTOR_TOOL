...
namespace Common.MSRTrackDecryptor
{
    ...
    public class MSRTrackDataDecryptor : IMSRTrackDataDecryptor
    {
        ...
        private byte[] decryptionSessionKey;
        ...
        public byte[] DecryptData(string initialKSN, string cipher)
        {
            ...
            decryptionSessionKey = CreateSessionKey(iPEK, baseKSN);

            //1234567890|1234567890|12345
            Debug.WriteLine($"DECRYPT KEY: {ConversionHelper.ByteArrayToHexString(decryptionSessionKey)}");
            Console.WriteLine($"DECRYPTOR: {ConversionHelper.ByteArrayToHexString(decryptionSessionKey)}");

            using (var tdes = new TripleDESCryptoServiceProvider())
            {
                tdes.Mode = CipherMode.CBC;
                tdes.Padding = PaddingMode.None;
                tdes.Key = decryptionSessionKey;
                tdes.IV = new byte[8];
        ...
        public byte[] DecryptTrackInformation(string initialKSN, string cipher, byte[] transactionIV)
        {
            byte[] finalBytes = null;
        ...
        /// <returns></returns>
        public MSRTrackDataPayload RetrieveTrackData(byte[] trackInformation)
        {
            MSRTrackDataPayload trackData = new MSRTrackDataPayload()
            {
                PANData = string.Empty,
                Name = string.Empty,
                ExpirationDate = string.Empty,
        ...
        public MSRTrackDataPayload RetrieveAdditionalData(byte[] trackInformation)
        {
            MSRTrackDataPayload trackData = new MSRTrackDataPayload()
            {
                PANData = string.Empty,
        ...
        /// <param name="trackInformation"></param>
        /// <returns></returns>
        public MSRTrackDataPayload RetrieveSREDTrackData(byte[] trackInformation)
        {
        ...
        public MSRTrackDataPayload RetrieveTrackEquivalentData(byte[] trackInformation)
        {
            byte[] track2EquivalentData = new byte[] { 0x57 };
            byte[] panEquivalentData = new byte[] { 0x5A };
            byte[] responseMessageTemplate = new byte[] { 0x80 };
            ...
            string decryptedTrackData = "";
            string panData = string.Empty;

            foreach (var tag in tags)
            {
                if (tag.Tag.SequenceEqual(track2EquivalentData))
                {
                    decryptedTrackData = ConversionHelper.ByteArrayToHexString(tag.Data);
                }
                else if (tag.Tag.SequenceEqual(panEquivalentData))
                {
                    panData = ConversionHelper.ByteArrayToHexString(tag.Data);
                }
            }

            // clean up track data
            Debug.WriteLine($"DECRYPTED _: {decryptedTrackData}");

            // expected format: PAN^D^EXPIRATIONDATE^SERVICECODE^DISCRETIONARY-DATA
            MatchCollection match = Regex.Matches(decryptedTrackData, @"([0-9]{16})D([0-9]{4})([0-9]{3})([0-9]{2})([0-9[A-F]{12})", RegexOptions.Compiled);
            ...
            if (match.Count == 1 && match[0].Groups.Count >= 5)
            {
                ...
                // PIN Verification
                trackData.PinVerificationCode = match[0].Groups[4].Value;
