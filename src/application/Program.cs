...
using Common.MSRTrackDecryptor;
using Common.OnlinePinDecryptor;
using Config;
...
namespace DeviceDecryptorTool
{
    ...
    class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            AppConfig configuration = SetupEnvironment.GetApplicationConfiguration();
        ...
        private static void MsrTrackDecryption(IConfiguration configuration, int index)
        {
            ...
            if (trackData.Count() > index)
            {
                ...
                try
                {
                    ...
                    MSRTrackDataPayload trackInfo = decryptor.RetrieveTrackData(trackInformation);
        ...
        private static void MsrTrackDecryption(MSRTrackData trackData)
        {
            // Is there a matching item?
            if (trackData is { })
            {
                string msrTrackKsn = trackData.KSN;
                string msrTrackIV = trackData.IV;
                string msrEncryptedTrackData = trackData.EncryptedTrackData;
                string msrDecryptedTrackData = trackData.DecryptedTrackData;

                try
                {
                    //1234567890|1234567890|12345
                    Console.WriteLine($"\r\n==== [ MSR TRACK DECRYPTION ] ====");
                    Logger.info("==== [ MSR TRACK DECRYPTION ] ====");

                    MSRTrackDataDecryptor decryptor = new MSRTrackDataDecryptor();

                    Debug.WriteLine($"KSN _______: {msrTrackKsn}");
                    Debug.WriteLine($"IV  _______: {msrTrackIV}");
                    Debug.WriteLine($"DATA ______: {msrEncryptedTrackData}");

                    WriteToConsoleAndLogger($"KSN      : {msrTrackKsn}");
                    WriteToConsoleAndLogger($"IV       : {msrTrackIV}");
                    WriteToConsoleAndLogger($"DATA     : {msrEncryptedTrackData}");

                    // decryptor in action: DUKPT data operation
                    byte[] dataTrackKey = decryptor.DecryptData(msrTrackKsn, msrEncryptedTrackData);

                    string decodedDataTrackKey = ConversionHelper.ByteArrayToHexString(dataTrackKey);

                    //1234567890|1234567890|12345
                    //WriteToConsoleAndLogger($"OUTPUT   : {decodedDataTrackKey}");
                    Debug.WriteLine($"OUTPUT ____: {decodedDataTrackKey}");

                    // decryptor in action: CIPHER 3DES/CBC/Padding ISO 7816-4
                    byte[] decryptedTrackInformation = decryptor.DecryptTrackInformation(msrTrackKsn, msrEncryptedTrackData, 
                        ConversionHelper.HexToByteArray(msrTrackIV));
                    Debug.WriteLine($"OUTPUT ____: {ConversionHelper.ByteArrayToHexString(decryptedTrackInformation)}");

                    //MSRTrackData trackInfo = decryptor.RetrieveAdditionalData(decryptedTrackInformation);
                    MSRTrackDataPayload trackInfo = decryptor.RetrieveTrackEquivalentData(decryptedTrackInformation);
                    ...
                    WriteToConsoleAndLogger($"EXPIRATE : {trackInfo.ExpirationDate}");
                    WriteToConsoleAndLogger($"SERV CODE: {trackInfo.ServiceCode}");

                    if (!string.IsNullOrEmpty(msrDecryptedTrackData))
                    {
                        byte[] expectedValue = ConversionHelper.HexToByteArray(msrDecryptedTrackData);
                        bool result = StructuralComparisons.StructuralEqualityComparer.Equals(expectedValue, decryptedTrackInformation);
                        WriteToConsoleAndLogger($"EQUAL    : [{result}]");
                    }
                    else
                    {
                        WriteToConsoleAndLogger("\nCOMPARISON NOT EXECUTED");
                    }

                    //MSRTrackData trackData = decryptor.RetrieveTrackData(trackInformation);
                    //WriteToConsoleAndLogger($"CHOLDER: [{trackData.Name}]");
                }
                catch (Exception e)
                {
                    WriteToConsoleAndLogger($"EXCEPTION: {e.Message}");
                }
            }
        }
        ...
        private static void InternalTesting()
        {
            try
            {
                foreach (var item in trackPayload)
                {
                    ...
                    MSRTrackDataPayload trackData = decryptor.RetrieveTrackData(trackInformation);
                    Console.WriteLine($"CHOLDER: [{trackData.Name}]");
                }
            }
            catch (Exception e)
        ...
        private static void WriteToConsoleAndLogger(string message)
        {
